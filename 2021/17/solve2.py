    ...
    for step in steps:
      if step == -1:
        ...
        while True:
          ...
          [velvectors.add((velx, vely)) for vely in range(ceil(velyrangebot), floor(velyrangetop) + 1, 1)]
          # print('{}: {} to {}, max {}'.format(newstep, velyrangebot, velyrangetop, len(velvectors)))
          newstep += 1
          if newstep > 1000:
            break
      else:
        ...
        [velvectors.add((velx, vely)) for vely in range(ceil(velyrangebot), floor(velyrangetop) + 1, 1)]
        # print('{}: {} to {}, max {}'.format(step, velyrangebot, velyrangetop, len(velvectors)))

  print(len(velvectors))
