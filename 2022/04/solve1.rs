...
fn main() {
    ...
    for file_path in &args[1..] {
        ...
        let assn_full_overlap = assn_pairs.into_iter()
                                        .map(|pair| if full_overlap(pair.0, pair.1) {1} else {0});
        let full_overlap_sum = assn_full_overlap.fold(0, |x, y| x + y);
        println!("{}", full_overlap_sum);
    }
}
