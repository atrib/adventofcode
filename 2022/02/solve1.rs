...
fn pair_score((opp, own): (i64, i64)) -> i64 {
    let shape_score = 1 + own;
    let outcome_score = if (opp + 1) % 3 == own { 6 }      // Win
                        else if opp == own      { 3 }      // Draw
                        else                    { 0 };     // Loss
...
fn to_int_rep(pairvec: Vec<&str>) -> (i64, i64) {
    ...
    let own = pairvec[1].chars().nth(0).unwrap();

    return (opp as i64 - 'A' as i64, own as i64 - 'X' as i64);
}
